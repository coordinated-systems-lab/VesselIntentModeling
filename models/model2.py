...
class model2(nn.Module):
	...
	def forward(self,sequence,dist_matrix,rb_matrix,cog_matrix,seq_mask,maxval,minval,maxVessels,return_context=False):
		...
		weightedInput=torch.FloatTensor(batchSize,maxVessels,self.sequence_length,self.hidden_size).to(device)
		for i in range(self.sequence_length):
			weightedHidden=self.hardwiredAttention(torch.stack(h_t),rb_matrix[:,:,i,...],dist_matrix[:,:,i,...],seq_mask[:,:,i],cog_matrix[:,:,i,:])
			weightedInput[:,:,i,:] = weightedHidden
		...
		for i in range(self.prediction_length):
			...
			for j in range(maxVessels):
				weightedHidden_j, context_vector_j= self.softAttention[j](weightedInput[:,j,...],weightedHidden[:,j,...],seq_mask[:,j,:])
				h_t[j],c_t[j]=self.lstm[j](previous_seq[:,j,...],weightedHidden_j,c_t[j])
				if return_context:
...
"""
def smoothen_domain(domain,box_pts):
	box = torch.ones(box_pts).float().to(device)
	box/=(box_pts)
	domain_smooth = # np.convolve(domain, box, mode='same')
